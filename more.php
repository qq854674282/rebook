...
$query="select class_id,major_id,column_name from info_column where column_id=$column_id";
    ...
    <div class="main_content">
        <div class="book_line">
            <div class="book_line_title">
                <a href="#" class="book_line_title_1 pull-left"><?php echo $column_name;?></a><!--搜索关键词或分类关键词-->
            </div>
            <div class="clearfix"></div>
			<?php
					//分页
					$pagesize = 20;
					if($major_id!=0){
						$sql_page="select 1 from book where status=0 and major_id=$major_id";
					}
					if($class_id!=0){
						$sql_page="select 1 from book where status=0 and class_id=$class_id";
					}
					...
					if($major_id!=0){
						$sql = "select book_id,book_name,book_price,zone,send_type,book_image,add_time from book where status=0 and major_id=$major_id order by add_time desc limit ".$pagestart.",".$pagesize;
					}
					if($class_id!=0){
						$sql = "select book_id,book_name,book_price,zone,send_type,book_image,add_time from book where status=0 and class_id=$class_id order by add_time desc limit ".$pagestart.",".$pagesize;
					}
					$rs = mysql_query($sql);
